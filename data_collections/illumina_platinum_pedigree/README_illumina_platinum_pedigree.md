...
Under this directory, the alignment data can be found in [data/CEU](ftp://ftp.1000genomes.ebi.ac.uk/vol1/ftp/data_collections/illumina_platinum_pedigree/data/CEU/), where it is organised by sample name (with both replicates present in the NA12882 directory). 

Under each sample, the alignment directory contains alignments generated by aligning the data with bwa mem to a reference genome comprising GRCh38, the HLA sequences and decoy sequences. This uses the alt aware approach described here:
